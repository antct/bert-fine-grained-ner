...
class CRF(nn.Module):
    ...
    def __init__(self, target_size, use_cuda=True, avg_batch=True):
        super(CRF, self).__init__()
        self.target_size = target_size
        self.SOS_IDX, self.EOS_IDX = -2, -1
        self.use_cuda = use_cuda
        self.avg_batch = avg_batch
        init_transitions = torch.zeros(self.target_size, self.target_size)
        init_transitions[:, self.SOS_IDX] = -1000.
        init_transitions[self.EOS_IDX, :] = -1000.
        if self.use_cuda:
    ...
    def _forward_alg(self, feats, mask=None):
        ...
        partition = inivalues[:, self.SOS_IDX,
                              :].clone().view(batch_size, tag_size, 1)
        ...
        cur_values = self.transitions.view(1, tag_size, tag_size).expand(
            batch_size, tag_size, tag_size) + partition.contiguous().view(
                batch_size, tag_size, 1).expand(batch_size, tag_size, tag_size)
        cur_partition = log_sum_exp(cur_values, tag_size)
        final_partition = cur_partition[:, self.EOS_IDX]
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        end_transition = self.transitions[:, self.EOS_IDX].contiguous().view(
