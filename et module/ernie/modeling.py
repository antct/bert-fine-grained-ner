...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, input_tensor_ent, attention_mask_ent):
        ...
        attention_output_ent = self.output_ent(
            self_output_ent, input_tensor_ent)
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_ent):
        ...
        hidden_states = self.intermediate_act_fn(
            hidden_states_+hidden_states_ent_)
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states_, input_tensor, input_tensor_ent):
        ...
        hidden_states_ent = self.LayerNorm_ent(
            hidden_states_ent + input_tensor_ent)
...
class BertLayerMix(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask):
        ...
        layer_output, layer_output_ent = self.output(
            intermediate_output, attention_output, attention_output_ent)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask):
        attention_output, attention_output_ent = self.attention(
            hidden_states, attention_mask, hidden_states_ent, attention_mask_ent)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask, output_all_encoded_layers=True):
        ...
        for layer_module in self.layer:
            hidden_states, hidden_states_ent = layer_module(
                hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask)
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output, candidate):
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
        prediction_scores_ent = self.predictions_ent(
            sequence_output, candidate)
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True):
        ...
        extended_ent_mask = extended_ent_mask.to(
            dtype=next(self.parameters()).dtype)  # fp16 compatibility
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        prediction_scores, seq_relationship_score, prediction_scores_ent = self.cls(
            sequence_output, pooled_output, candidate)
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            ent_ae_loss = loss_fct(
                prediction_scores_ent.view(-1, candidate.size()[0]), ent_labels.view(-1))
...
class BertForEntityTyping(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, labels=None):
        _, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False)
...
class BertForNQ(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, choice_mask=None, labels=None):
        ...
        flat_input_ent = input_ent.view(-1,
                                        input_ent.size(-2), input_ent.size(-1))
        ...
        _, pooled_output = self.bert(flat_input_ids, flat_token_type_ids, flat_attention_mask,
                                     flat_input_ent, flat_ent_mask, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        ...
        reshaped_logits = torch.cat(
            [null_socre, reshaped_logits], -1) + choice_mask
...
class BertForTokenClassification(PreTrainedBertModel):
    ...
    Inputs:
        ...
        `token_type_ids`: an optional torch.LongTensor of shape [batch_size, sequence_length] with the token
            types indices selected in [0, 1]. Type 0 corresponds to a `sentence A` and type 1 corresponds to
            a `sentence B` token (see BERT paper for more details).
        `attention_mask`: an optional torch.LongTensor of shape [batch_size, sequence_length] with indicesa
            selected in [0, 1]. It's a mask to be used if the input sequence length is smaller than the max
            input sequence length in the current batch. It's the mask that we typically use for attention when
            a batch has varying length sentences.
        `labels`: labels for the classification output: torch.LongTensor of shape [batch_size]
